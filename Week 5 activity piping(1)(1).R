...
write.csv(mysample, "D:/laplap_/day1cpt/McDaniel/ana505/L5dataset.csv", row.names = FALSE)
...
mysample4 <- mysample |> arrange(airline) %>%
  subset(incidents_85_99>10) %>%
  mutate(seats = avail_seat_km_per_week) %>%
  select(incidents_00_14, incidents_85_99) %>%
  summarize(incidents_00_14, incidents_85_99)
